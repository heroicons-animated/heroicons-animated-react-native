...
export type FilmIconHandle = IconHandle;

const FilmIcon = forwardRef<FilmIconHandle, IconProps>(
  ({ size = 28, color = "currentColor", strokeWidth = 1.5, style, controlled, onPress }, ref) => {
    const rotate = useSharedValue(0);

    const startAnimation = useCallback(() => {
      rotate.value = withSequence(withTiming(0, { duration: 83 }), withTiming(-2, { duration: 83 }), withTiming(2, { duration: 83 }), withTiming(-1, { duration: 83 }), withTiming(1, { duration: 83 }), withTiming(0, { duration: 83 }));
    }, [rotate]);
