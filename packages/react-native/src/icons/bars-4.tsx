...
export type Bars4IconHandle = IconHandle;

const Bars4Icon = forwardRef<Bars4IconHandle, IconProps>(
  ({ size = 28, color = "currentColor", strokeWidth = 1.5, style, controlled, onPress }, ref) => {
    ...
    const stopAnimation = useCallback(() => {
      ...
      b3.value = withTiming(0, { duration: 200 });
    }, [b0, b1, b2, b3]);
    ...
    const b0Props = useAnimatedProps(() => {
      ...
      return { d: `M${start.toFixed(3)} 5.25h${length.toFixed(3)}` };
    });
    const b1Props = useAnimatedProps(() => {
      const start = 3.75 + 3.3 * b1.value;
      const length = 16.5 - 6.6 * b1.value;
      return { d: `M${start.toFixed(3)} 9.75h${length.toFixed(3)}` };
    });
    const b2Props = useAnimatedProps(() => {
      const start = 3.75 + 3.3 * b2.value;
      const length = 16.5 - 6.6 * b2.value;
      return { d: `M${start.toFixed(3)} 14.25h${length.toFixed(3)}` };
    });
    const b3Props = useAnimatedProps(() => {
      const start = 3.75 + 3.3 * b3.value;
      const length = 16.5 - 6.6 * b3.value;
      return { d: `M${start.toFixed(3)} 18.75h${length.toFixed(3)}` };
    });
