...
export type TableCellsIconHandle = IconHandle;

const TableCellsIcon = forwardRef<TableCellsIconHandle, IconProps>(
  ({ size = 28, color = "currentColor", strokeWidth = 1.5, style, controlled, onPress }, ref) => {
    const scale = useSharedValue(1);
    const opacity = useSharedValue(1);

    const startAnimation = useCallback(() => {
      scale.value = withSequence(withTiming(1, { duration: 100 }), withTiming(0.95, { duration: 100 }), withTiming(1.02, { duration: 100 }), withTiming(1, { duration: 100 }));
      opacity.value = withSequence(withTiming(1, { duration: 100 }), withTiming(0.8, { duration: 100 }), withTiming(1, { duration: 100 }), withTiming(1, { duration: 100 }));
    }, [scale, opacity]);
