...
export type FolderOpenIconHandle = IconHandle;

const FolderOpenIcon = forwardRef<FolderOpenIconHandle, IconProps>(
  ({ size = 28, color = "currentColor", strokeWidth = 1.5, style, controlled, onPress }, ref) => {
    const rotate = useSharedValue(0);

    const startAnimation = useCallback(() => {
      rotate.value = withSequence(withTiming(0, { duration: 120 }), withTiming(-8, { duration: 120 }), withTiming(6, { duration: 120 }), withTiming(-4, { duration: 120 }), withTiming(0, { duration: 120 }));
    }, [rotate]);
